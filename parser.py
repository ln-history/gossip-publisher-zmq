# Copied from ln-research/topology. Written by Christian Decker
...
def parse(b):
    ...
    try:
        ...
        print(f"Error: Could not read message type. {e}")
        return None
    ...
    if typ not in parsers:
        print(f"Warning: No parser registered for type {typ}")
        return None

    try:
        return parsers[typ](b)
    except Exception as e:
        print(f"Error parsing message type {typ}: {e}")
        return None
...
def parse_channel_announcement(b):
    try:
        ...
        try:
            ...
            print("Error: Could not read feature length in ChannelAnnouncement")
            ca.features = b""
        ...
        try:
            ...
            print(f"Error parsing ChannelAnnouncement fields: {e}")
            return None

        return ca
    except Exception as e:
        print(f"Unexpected error in parse_channel_announcement: {e}")
        return None
...
def parse_channel_update(b):
    try:
        ...
        try:
            ...
            print(f"Error parsing ChannelUpdate fields: {e}")
            return None

        t = b.read(8)
        cu.htlc_maximum_msat = struct.unpack("!Q", t)[0] if len(t) == 8 else None

        return cu
    except Exception as e:
        print(f"Unexpected error in parse_channel_update: {e}")
        return None
...
def parse_node_announcement(b):
    try:
        ...
        na = NodeAnnouncement()
        na.signature = b.read(64)  # Read node signature (64 bytes)

        try:
            (flen,) = struct.unpack("!H", b.read(2))  # Read feature length (2 bytes)
            na.features = b.read(flen)  # Read features based on length
            (na.timestamp,) = struct.unpack("!I", b.read(4))  # Read timestamp (4 bytes)
            na.node_id = b.read(33)  # Read node ID (33 bytes)
            na.rgb_color = b.read(3)  # Read RGB color (3 bytes)
            na.alias = b.read(32)  # Read alias (32 bytes)
        except struct.error as e:
            print(f"Error parsing NodeAnnouncement fields: {e}")
            return None

        try:
            (alen,) = struct.unpack("!H", b.read(2))  # Read address length (2 bytes)
            abytes = io.BytesIO(b.read(alen))  # Read address bytes
            na.addresses = []
            while True:
            ...
            print(f"Error parsing NodeAnnouncement addresses: {e}")
            return None

        return na
    except Exception as e:
        print(f"Unexpected error in parse_node_announcement: {e}")
        return None
    


print(parse_node_announcement(io.BytesIO(bytes.fromhex(""))))